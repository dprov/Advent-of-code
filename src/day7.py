...
def find_min_dir_size_greater_than(root: FilesystemNode, min_valid_size: int) -> int:
    file_sizes = [
        node.file_size for node in anytree.PreOrderIter(root) if node.file_size >= min_valid_size
    ]
...
if __name__ == "__main__":
    root = build_filesystem_from_input("input/day7")
    # print_tree(root)

    print("Part I")
    print(sum_dir_sizes(root, max_dir_size=100000))

    print("Part II")
    print(get_dir_to_remove_size(root, total_space=int(7e7), min_space_needed=int(3e7)))
